...
# Fix f=1.32 (from Mathematica best) and scan only phi and M4
# This tells us the w vs Omega trade-off clearly
f = 1.31999  # fixed from Mathematica

phi_values = np.linspace(0.40, 1.20, 10)
M4_values  = np.linspace(4.0e-8, 1.2e-7, 10)
...
print(f"Fixed f = {f}")
print(f"Target: w={target_w}  Omega={target_Omega}  dw/dN={target_dwdN}")
print(f"Running {total} CLASS evaluations...\n")
print(f"{'phi_ini':>10}  {'scf_M4':>12}  {'w':>10}  {'Omega':>10}  {'dw/dN':>10}  {'score':>12}")
print("-" * 72)
...
for M4 in M4_values:
    for phi_ini in phi_values:
        ...
        count += 1
        print(f"  [{count}/{total}] phi={phi_ini:.3f}  M4={M4:.2e} ...", end="\r")
        sys.stdout.flush()
...
print("\n=== BEST RESULT (f fixed at 1.31999) ===")
if best_result:
