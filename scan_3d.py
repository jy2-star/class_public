...
# 3D scan over phi_ini, scf_M4, scf_f
phi_values = np.linspace(0.8, 1.2, 6)
M4_values  = np.linspace(6.0e-8, 1.0e-7, 6)
f_values   = np.linspace(0.8, 1.32, 6)
    ...
    for M4 in M4_values:
        for phi_ini in phi_values:
            ...
            try:
                ...
                # Compute dw/dN at today
                z   = data[:, 0]
                a   = 1.0 / (1.0 + z)
                N   = np.log(a)
                w   = data[:, 15]
                dw_dN_arr = np.gradient(w, N)
                dw_dN = dw_dN_arr[-1]

                # Combined score for all 3 constraints
                ...
                if (abs(w_today   - target_w)     < 0.01 and
                    abs(Omega_phi - target_Omega)  < 0.01 and
                    abs(dw_dN     - target_dwdN)   < 0.05):
                    print(f"{phi_ini:>10.4f}  {M4:>12.3e}  {f:>8.4f}  "
                          f"{w_today:>10.4f}  {Omega_phi:>10.4f}  "
                          f"{dw_dN:>10.4f}  {score:>12.6e}  <-- GOOD")
...
if best_result:
    ...
    print(f"phi_ini   = {phi_ini:.8f}")
    print(f"scf_M4    = {M4:.8e}")
    print(f"scf_f     = {f:.8f}")
    print(f"w_today   = {w_today:.6f}  (target: {target_w})")
    print(f"Omega_phi = {Omega_phi:.6f}  (target: {target_Omega})")
    print(f"dw/dN     = {dw_dN:.6f}  (target: {target_dwdN})")
